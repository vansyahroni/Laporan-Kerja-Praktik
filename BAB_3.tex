...
Pada robot SCARA, sudut dari \textit{joint} yang dicari merupakan \textit{joint} dari \textit{shoulder} dan juga \textit{elbow.} Kedua joint tersebut dapat ditemukan dengan melalui persamaan \textit{pythagoras} dan juga hujum \textit{cosinus}. Pada Gambar 3.17 terlihat bahwa sudut shoulder merupakan besar sudut diantara lengan shoulder dan juga sumbu x dan sudut elbow merupakan besar sudut antara lengan elbow dengan garis bantu dari lengan shoulder. Keduanya dapat ditentukan besar nilainya melalui beberapa pesamaan dari cosinus dan juga hukum pyhtagoras.LS merupakan panjang lengan shoulder dan LE merupakan panjang lengan dari elbow. Serta SS merupakan sudut dari shoulder dan SE merupakan sudut dari elbow.  

\begin{enumerate}
	\item Dengan menggunakan hukum cosinus, didapatkan sebuah persamaan \ref{cosinus1}
	\begin{equation}
	(x^2+y^2)=LS^2+LE^2-2LSLEcos(180-SE)
	\label{cosinus1}
	\end{equation}
	\item Pada persamaan \ref{cosinus1} terdapat cos(180-SE) yang pada hukum cosinus dapat diubah menjadi -cos(SE), maka persamaan setelahnya ditunjukkan pada persamaan \ref{cosinus2}
	\begin{equation}
	(x^2+y^2)=LS^2LE^2+2LSLEcos(SE)
	\label{cosinus2}
	\end{equation}
	\item Merujuk pada persamaan \ref{cosinus2} maka dapat dikelompokkan agar SE dapat diketahui menjadi seperti persamaan \ref{cosinus3}
	\begin{equation}
	cos(SE)=(x^2+y^2-LS^2-LE^2)/(2LSLE)
	\label{cosinus3}
	\end{equation}
